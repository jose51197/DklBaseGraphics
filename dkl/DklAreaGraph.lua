...
require "dkl/DklAreaPlot"

function _DklAreaGraph(x,y,w,h,dataY,color,mode,event,tickInc,
	...
	if (mode == VERTICAL) then
		...
		selection = __DklAreaPlot(w,0,dataY,dataX,color,rateY,
...
function DklAreaGraph(options)
	local state = options.state or {id=0,selection={}}
	state.id = state.id + 1
	local id = state.id
	state.selection[id] = _DklAreaGraph(options.x or 0, options.y or 0, 
