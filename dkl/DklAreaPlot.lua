...
function __DklAreaPlot(w,h,dataX,dataY,color,rateX,rateY,mode,selected,eventCode,label)
	...
	for i=1,n do
		if (i==1) then
			...
			if (mode == VERTICAL) then
				vertex(dataX[i]*rateX,h)
			else
				vertex(0,h-dataY[i]*rateY)
			end
		end
...
function _DklAreaPlot(x,y,w,h,dataX,dataY,color,shape,size,mode,event,
	...
	if (mode == VERTICAL) then
		selection = __DklAreaPlot(w,h,dataX,dataY,color,rateX,
											rateY,mode,selected,event,label)
	else
		selection = __DklAreaPlot(w,h,dataY,dataX,color,rateY,
...
function DklAreaPlot(options)
	...
	state.selection[id] = _DklAreaPlot(options.x or 0, options.y or 0, 
